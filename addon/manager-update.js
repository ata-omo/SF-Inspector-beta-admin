...
class Model {
  ...
  constructor(sfHost, args) {
    ...
    // declaration for manager update
    this.empNoUserIdMap = new Map(); // stores a mapping of emp No.(from WD report) to User ID (query sf)
    this.workerEmpNoColIndex = 0; // column no. of the users employee no. in WD report
    this.managerEmpNoColIndex = 9; // column no. of the managers employee no. in WD report
    this.reportData = null; // store the copy of data with header
    this.hasPastedData = false;
    this.isPreprocessing = false;
    this.preprocessingComplete = false;
    this.preprocessingError = "";

    // for querying users efficiently
    this.queryProgress = {};
    this.queryStatus = "";
  ...
  setData(text) {
    ...
    // console.log("unparsed data", text);
    // console.log("parsed data", data);
    this.reportData = data.map(row => [...row]);
    // console.log("report data (with header)", this.reportData);
    // console.log("test report data",this.reportData);
    ...
    // for manager update (enable the process data button once data is pasted)
    this.hasPastedData = true;
    this.isPreprocessing = false;
    this.preprocessingComplete = false;
    this.preprocessingError = "";
  ...
  // for effecient querying start

  escapeSoqlValue(value){
    return String(value == null ? "": value).replace(/([\\'])/g, "\\$1");
  }

  setQueryEndpoint(query){ // similar to setQueryMethod() from data-export.js

    const method = "query";
    const queryParams = "/?q=" + encodeURIComponent(query);

    const endpoint = "/services/data/v" + apiVersion + "/" + method + queryParams;
  ...
  runQuery(query){
    ...
    function batchHandler(batch){
      return batch.catch(err => {
        ...
        if(data && !data.done && data.nextRecordsUrl){
          vm.queryStatus = `Retriving records ${records.length} of ${data.totalSize}`;
          vm.didUpdate();

          return batchHandler(sfConn.rest(data.nextRecordsUrl,{progressHandler: vm.queryProgress}));
        }
    ...
    const endpoint = vm.setQueryEndpoint(query);

    return batchHandler(sfConn.rest(endpoint,{progressHandler: vm.queryProgress}));


  }

  // for effecient querying end
  ...
  isWorking() {
    return this.activeBatches != 0 || this.isProcessingQueue || this.isPreprocessing;
  }
  ...
  // for manager update start

  canProcessData() {
    return (
        this.hasPastedData
        && !this.isPreprocessing
  ...
  canRunUpdate() {
    return (
        ...
        && this.importData.counts.Queued > 0
        && !this.invalidInput()
    );
  }
  ...
  canCopy() {
    return (
        this.preprocessingComplete
        && this.importData.taggedRows != null
    );
  }

  // for manager updated end
...
//   }
  ...
  createProcessedMappingTable(data){
    // create emp no. : user id mapping
    let repData = this.reportData.map(row => [...row]); // original report
    let hdr = repData.shift();
    let userByEmpNo = new Map();
    for(const usr of data){
      let empNo = String(usr.EmployeeNumber).trim();
      let userid = String(usr.Id).trim();
      userByEmpNo.set(empNo,userid);
    }
    ...
    // inserting ids in the report rows
    for(let row of repData){
      let empNo = String(row[this.workerEmpNoColIndex]).trim();
      let mgrNo = String(row[this.managerEmpNoColIndex]).trim();

      let empUid = userByEmpNo.has(empNo) ? userByEmpNo.get(empNo) : "";
      let mgrUid = userByEmpNo.has(mgrNo) ? userByEmpNo.get(mgrNo) : "";

      row.push(empUid);
      row.push(mgrUid);

      if(empUid) processedRows.push([empUid,mgrUid,String(row[0]).trim(),String(row[1]).trim(),String(row[7]).trim(),String(row[6]).trim(),"Queued",empUid,"",""]);
      
    }

    const headerNames = ["Id","ManagerId","_EmployeeNumber","_EmployeeName","_OldManager","_NewManager","__Status","__Id","__Action","__Errors"];

    console.log("processed mapping table: ",processedRows);
    const header = headerNames.map((name,index) => this.makeColumn(name,index));

    this.updateResult(null);
    this.updateResult({header,data: processedRows});
    this.refreshColumn();
    this.updateResult(this.importData.importTable);
  ...
  doProcessData(){
    ...
    if(this.reportData.length < 2){
        this.isPreprocessing = false;
        this.preprocessingError = "No data is available to preprocess";
        this.dataError = this.preprocessingError;
        this.didUpdate();
        return;
    }

    const uniqueEmpNo = new Set();

    const dataRows = this.reportData.slice(1);

    for(const row of dataRows){
        const workerEmpNo = String(row[this.workerEmpNoColIndex] == null ? "" : row[this.workerEmpNoColIndex]);
        const managerEmpNo = String(row[this.managerEmpNoColIndex] == null ? "" : row[this.managerEmpNoColIndex]);

        if(workerEmpNo) uniqueEmpNo.add(workerEmpNo);
        if(managerEmpNo) uniqueEmpNo.add(managerEmpNo);
    }

    const empNums = Array.from(uniqueEmpNo);

    if(empNums.length === 0) {

        this.isPreprocessing = false;
        this.preprocessingError = "Given Employees and Managers not found in Kantata";
        this.dataError = this.preprocessingError;
    ...
    const processingPromise = this.queryUsersForIds(empNums).then(
        // putting everything back
        instreamData => {
            this.createProcessedMappingTable(instreamData);
            this.isPreprocessing = false;
        ...
        this.isPreprocessing = false;
        this.preprocessingComplete = false;
        this.preprocessingError = "Failed to process";
        this.dataError = this.preprocessingError;
        this.didUpdate();
    });

        
    this.spinFor(processingPromise);
    this.didUpdate();
  ...
  async queryUsersForIds(empNums){

    const userRecords = [];

    const numChunks = this.chunkArray(empNums,100);

    // querying the db

    for(const chunk of numChunks){
        const clause = chunk.map(emp => "'"+this.escapeSoqlValue(emp)+"'").join(",");

        const soql = "select "+"EmployeeNumber, "+"Id, "+"Name "+"from User "+"where isActive = true and EmployeeNumber IN ("+clause+")";
        const qry = "select EmployeeNumber, Id, Name from User where EmployeeNumber IN ("+clause+")";

        console.log("soql qry1 ",soql);
        console.log("soql qry2", qry);

        const results = await this.runQuery(soql);
        userRecords.push(...results);
    }

    console.log("all users: ",userRecords);
...
class App extends React.Component {
  constructor(props) {
    ...
    this.onDoProcessDataClick = this.onDoProcessDataClick.bind(this);
  ...
  // for manager update (process pasted data)

  onDoProcessDataClick(e){
    e.preventDefault();
    let {model} = this.props;
    model.doProcessData();
    model.didUpdate();
      ...
      h("div", {className: "area import-actions"},
        h("div", {className: "conf-line"},
          h("div", {className: "flex-wrapper"},
            h("button", {onClick: this.onDoProcessDataClick, disabled: !model.canProcessData(), className: "highlighted"}, model.isPreprocessing ? "Processing...": "Process Data"),
            h("button", {onClick: this.onDoImportClick, disabled: !model.canRunUpdate(), className: "highlighted"}, "Run " + model.importActionName),
